...
class DexEVM(DexClient):
    ...
    async def get_quote(
        ...
        Args:
            ...
            sell_address (str, optional): The address of the sell token. Defaults to None.
            sell_symbol (str, optional): The symbol of the sell token. Defaults to None.
        ...
        """
        try:
            logger.debug("evm quote {} {} {} {}", buy_address, buy_symbol, sell_address, sell_symbol)
            buy_token = await self.resolve_buy_token(buy_address=buy_address, buy_symbol=buy_symbol)
            sell_token = await self.resolve_sell_token(sell_address=sell_address, sell_symbol=sell_symbol)
            amount = str(amount * (10**sell_token.decimals))
            logger.debug(f"evm quote {buy_token.address} {sell_token.address} {amount}")

            # return self.client.get_quote(buy_token.address, sell_token.address, amount)
